...
class SonarQubePluginManagerTest {
    ...
    void beforeEach() {
        ...
        List.of(rootProject, subProject).forEach(project -> project.getPlugins().apply(SonarQubePluginManager.class));
    }

    @Test
    @DisplayName("Should apply plugins to root project")
    void shouldApplyPluginsToRootProject() {
        assertThat(rootProject.getPlugins().hasPlugin(SonarQubePluginManager.class)).isTrue();
    ...
    void shouldNotApplyPluginsToNonRootProject() {
        assertThat(subProject.getPlugins().hasPlugin(SonarQubePluginManager.class)).isTrue();
